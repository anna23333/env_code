...
void ABTD_affinity_init(void)
{
    ...
    printf("HAVE_PTHREAD_SETAFFINITY_NP\n");
    int i;
    int num_cores = 0;
   ...
   // for (i = 0; i < CPU_SETSIZE; i++) {
    ...
    printf("gp_ABTI_global->num_cores=%d=num_cores=%d\n", gp_ABTI_global->num_cores, num_cores);
    ...
    printf("don not support ES affinity.\n");
...
int ABTD_affinity_set(ABTD_xstream_context ctx, int rank)
{
    ...
    if(rank < 20)
        cpuset = ABTD_affinity_get_cpuset_for_rank(rank);
    else 
        cpuset = ABTD_affinity_get_cpuset_for_rank(rank-20);
