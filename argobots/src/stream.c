...
void ABT_read_from_pipe(int fd_fifo)
{

}

void ABT_from_argo_get_pipe(char *write_buffer)
{
    //get the argo core info, write into the write_buffer.
//    printf("es%s\n", gp_ABTI_global->es);
    strcpy(write_buffer, gp_ABTI_global->es);
}

void ABT_from_pipe_set_argo(char *buffer)
{
   // got the distribution info, set the argo migration setting.
   // buffer is the setting from the pipe read.
 
    strcpy(gp_ABTI_global->es, buffer);
    
//    for(int i = 0; i< 40; i++)
//        gp_ABTI_global->empty_list[i] = buffer[i];
}

void ABT_set_es()
{//every loop
    int id = rand() % 39;
    ABTI_xstream *p_es = gp_ABTI_global->p_xstreams[id];
    ABT_xstream_state state;
    ABT_xstream es = ABTI_xstream_get_handle(p_es);
    ABT_xstream_get_state(es, &state);

    if(state == 2) gp_ABTI_global->es[id] = '2';
    else if(state == 1) gp_ABTI_global->es[id] = '1';
    else if(state == 0) gp_ABTI_global->es[id] = '0';
    else gp_ABTI_global->es[id] = '9';
}

void ABT_set_pool(ABT_sched sched)
{
    size_t size;
    int pool_id;
...
void ABT_pipe_set_info(ABT_sched sched)
{
    ...
    if(state == 2) 
        gp_ABTI_global->es[id] = '2';
    else if(state == 1)
        gp_ABTI_global->es[id] = '1';
    else if(state == 0)
        gp_ABTI_global->es[id] = '0';
    else
        gp_ABTI_global->es[id] = '9';

    printf("es_id=%d, state=%d ; pool_id=%d, pool_size=%ld, es=%s\n", id, state,pool_id, size, gp_ABTI_global->es);
}

void ABT_shenjingbing()
{
    int i;
}
...
int ABT_my_migrate(ABT_pool *pools, ABT_sched sched, int *run_cnt)
{
    ...
    if(gp_ABTI_global->empty_list[target] == 1)
    {
        ...
        {
        //    ABT_pool_migration_second();
            target_second = rand() % 39 +2;
            ABT_pool_get_size(pools[target_second], &size);
            if(size > 0)
            {
                ABT_pool_pop(pools[target_second], &unit);
                ABT_unit_set_associated_pool(unit, pools[target_second]);
                ABT_pool_push(pools[target_second], unit);
            }
