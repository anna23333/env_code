...
int ABT_init(int argc, char **argv)
{
    ...
    gp_ABTI_global->state_list = (char *)malloc(81*sizeof(char));
    gp_ABTI_global->es = (char *)malloc(41*sizeof(char));

    for(int j=0; j<40; j++)
        gp_ABTI_global->es[j] = 'a';
    gp_ABTI_global->es[40] = '\0';

    for(int i = 0; i < 80; i++)
    {
        gp_ABTI_global->state_list[i] = '2';
        gp_ABTI_global->empty_list[i] = 9;
