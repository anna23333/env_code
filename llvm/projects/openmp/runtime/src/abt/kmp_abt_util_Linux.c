...
static void __kmp_abt_sched_run_es0(ABT_sched sched)
{
    char *fifo_name1 = "/home/anna/part2/es_communication/pipe1";
    char *fifo_name3 = "/home/anna/part2/es_communication/pipe3";
...
/*
    ...
    if(access(fifo_name3, F_OK) == -1) {
        ...
        if(res != 0) {
            fprintf(stderr, "write: could not create fifo %s\n", fifo_name3);
        }
    }

    pipe_read = open(fifo_name1, O_RDONLY | O_NDELAY);
    if(pipe_read == -1)
        printf("read: open failed\n");

    pipe_write = open(fifo_name3, O_WRONLY);
    if(pipe_write == -1)
...
*/
    ...
    while (1) {
        ...
        //test which es is free.
        ABT_pipe_set_info(sched);

        if(freq % 50 == 0) {

            ABT_from_argo_get_pipe(write_buffer);
            res = write(pipe_write, write_buffer, strlen(write_buffer)+1);

        }

        if(freq % 10000  == 0)
        {
            //read the core distribution info from pipe.
            res = read(pipe_read, read_buffer, 101);

            //got the distribution info, set the argo migration setting.
            ABT_from_pipe_set_argo(read_buffer);

            //get the argo core info, write into the write_buffer.
            ABT_from_argo_get_pipe(write_buffer);

            //wite the write_buffer to pipe.
            res = write(pipe_write, write_buffer, strlen(write_buffer)+1);
         }
...
//        }
    ...
    gettimeofday(&end, NULL);
    printf("end of run_es: %ld:%ld\n", end.tv_sec, end.tv_usec);

//    printf("end of run_es\n");
    free(pools);
}
